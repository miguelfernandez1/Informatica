...
void nearestFriendEnemy(const vector<FicheroARFF*>& train, const FicheroARFF& actual, int& posFriend, int& posEnemy){
    ...
    for(int i = 0; i < train.size(); i++){
        if(&actual != train[i]){
            ...
            else if(distancia_actual < mejor_distancia_e){
...
void relief(const vector<FicheroARFF*>& train, vector<double>& w){
    int pos_nearest_friend, post_nearest_enemy;
    double w_max = -1.0;

    for(int i = 0; i < train.size(); i++){
        nearestFriendEnemy(train, *train[i], pos_nearest_friend, post_nearest_enemy);
        for(int j = 0; j < w.size(); j++){
            w[j] += fabs(train[i] -> traits[j] - train[post_nearest_enemy] -> traits[j]);
